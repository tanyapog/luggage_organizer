    ...
    if (e is Exception) {
      if (e is PlatformException) {
        ...
        if (eMessage.contains('PERMISSION_DENIED')) {
          return TripFailure.insufficientPermission(e, "Insufficient Permission");
        } else if (eMessage.contains('NOT_FOUND')) {
          return TripFailure.unableToUpdate(e, "Unable to update trip. Was it deleted from another device?");
        } else {
          return TripFailure.unexpected(e, "Unexpected platform error: \n$eMessage");
        }
      } else {
        return TripFailure.unexpected(e, "Unexpected error: \n$e");
      }
    } else {
      return TripFailure.unableToProcess(e, "Not acceptable: \n $e");
    }
  }
}
